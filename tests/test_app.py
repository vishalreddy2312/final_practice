...
def test_grid_analysis():
    ...
    for j in range(height):
        ...
        for i in range(width):
            row.append(app.generate_people_info())
        grid.append(row)
    new_grid, victim_name, victim_count, vic_plot, time_step = app.grid_analysis(grid, height, width)
...
def test_finding_victims():
    ...
    def test_sub_2( victim_name, victim_count, vic_plot):
        return True if victim_name != [] or victim_name == [] or victim_count != 0 or victim_count== 0 or vic_plot != [] or vic_plot == [] else False
